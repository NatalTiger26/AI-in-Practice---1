...
"""
    ...
    JUST ONE \\ INSTTEAD OF THE 2 AS WE ARE WRITING HERE
...
class Ticket5(BaseModel):
    ...
    urgency: int = Field(ge=1, 
                         le=5, 
                         description = (
            "Urgency 1-5."
            "5 = an emergency in progress or a hard same-day deadline."
            "3 = something is already stuck and the customer is waiting."
...
class Ticket6(BaseModel):
    ...
    def _clean(cls, v):
        if v == None:
            return
        elif v.lower().strip() in ["", "null", "none", "n/a"]:
            return None
        else:
            return v.strip()
...
model_validate() validates a dictionary/object against the model, 
ValidationError.errors() gives you structured information about what failed.
...
class TicketRecord(BaseModel):
    ...
    evidence: str = Field(max_length = 200, 
                          description = ("evidence is a string with a maximum of 200 characters"),)
    category: Literal[CATEGORIES] = Field(description="the category can only be one of the followingbilling, claims, policy_change, technical, complaint or information")
    urgency: int = Field(ge=1, 
                             le=5, 
                             description = (
                "Urgency 1-5."
    ...
    policy_number: str | None = Field(default=None, 
                                      pattern=r"^AUR-\d{7}$",
                                      description=("Policy number, It has to match r\"^AUR-\\d{7}$\" otherwise None."),) 
    @field_validator("policy_number", mode="before")
    @classmethod
    def _clean(cls, v):
        if v == None:
