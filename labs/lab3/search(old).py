...
def sweep_chunking() -> None:
    ...
    print('='*100)
    # print()
    print(f'testing various strategies like - {STRATEGIES.keys()}')
    print()
    for strat_keys in STRATEGIES.keys():
        chunks = build_chunks(corpus, strat_keys, size, overlap=150)
        print(f"corpus: {len(corpus)} docs -> {len(chunks)} chunks "
            f"(mean {statistics.fmean(len(c) for c in chunks):.0f} chars)")
        r = DenseRetriever(chunks)
        m = evaluate(r, questions)
        m["overall_metric"] = calc_final_metric(m)
        strat_sweep[f"{strat_keys} {size} dense"] = m
    print()
    print(table(strat_sweep, cols = ("hit_rate@1", "hit_rate@5", "recall@5", "mrr", "ndcg@10", "latency_p95_ms", "overall_metric")))
    print()
    best_strat = max(strat_sweep, key=lambda k: strat_sweep[k]["overall_metric"]).split(' ')[0]
    ...
    size_sweep={}
    for size in sizes:
        chunks = build_chunks(corpus, test_strat, size, overlap=150)
        print(f"corpus: {len(corpus)} docs -> {len(chunks)} chunks "
            f"(mean {statistics.fmean(len(c) for c in chunks):.0f} chars)")
        r = DenseRetriever(chunks)
        m = evaluate(r, questions)
        m["overall_metric"] = calc_final_metric(m)
        size_sweep[f"{strat_keys} {size} dense"] = m
    print()
    print(table(size_sweep, cols = ("hit_rate@1", "hit_rate@5", "recall@5", "mrr", "ndcg@10", "latency_p95_ms", "overall_metric")))
    print()
    best_size = max(size_sweep, key=lambda k: size_sweep[k]["overall_metric"]).split(' ')[1]
    ...
    m = evaluate(r, questions)
    m["overall_metric"] = calc_final_metric(m)
    markdown_sweep[f"{strat} {size} dense {prefix[0]}"] = m
    ...
    m = evaluate(r, questions)
    m["overall_metric"] = calc_final_metric(m)
    markdown_sweep[f"{strat} {size} dense {prefix[1]}"] = m
    print()
    print(table(markdown_sweep, cols = ("hit_rate@1", "hit_rate@5", "recall@5", "mrr", "ndcg@10", "latency_p95_ms", "overall_metric")))
    print()
    best_prefix = max(markdown_sweep, key=lambda k: markdown_sweep[k]["overall_metric"]).split(' ')[-1]
...
def sweep_retrieval() -> None:
    ...
    print()
    retriever_def = {'dense':DenseRetriever,
                 'bm25':Bm25Retriever,
    ...
    for ret in ['dense', 'bm25', 'hybrid']:
        ...
        print(f"corpus: {len(corpus)} docs -> {len(chunks)} chunks "
            f"(mean {statistics.fmean(len(c) for c in chunks):.0f} chars)")
        r = retriever_def[ret](chunks)
        m = evaluate(r, questions)
        m["overall_metric"] = calc_final_metric(m)
        retrival_sweep[f"{strat} {size} {ret}"] = m
    print()
    print(table(retrival_sweep, cols = ("hit_rate@1", "hit_rate@5", "recall@5", "mrr", "ndcg@10", "latency_p95_ms", "overall_metric")))
    print()
    best_ret = max(retrival_sweep, key=lambda k: retrival_sweep[k]["overall_metric"]).split(' ')[-1]
